...
def getDefinition(argsListList, caller="vim"):
    ...
    if (caller == "bash"):
        ...
        while True:
            ...
            if (re.match("^" + bsdcvResult, line)):
                # Re-format bsdcvResult
                replacedStr = re.sub("^(" + bsdcvResult + ")",
                        specialSubStr, line, flags=re.IGNORECASE)

            elif (re.match("^" + bsdcvDictName, line)):
                # Re-format bsdcvDictName
                replacedStr = re.sub("^(" + bsdcvDictName + ")",
                        underlinedSubStr, line, flags=re.IGNORECASE)

            elif (re.match("^" + bsdcvWord, line)):
                # Re-format bsdcvWord
                replacedStr = re.sub("^(" + bsdcvWord + ")", preProcSubStr,
                        line, flags=re.IGNORECASE)

            elif (re.match("^" + bsdcvWordType, line)):
                # Re-format bsdcvWordType
                replacedStr = re.sub("^(" + bsdcvWordType + ")",
                        statementSubStr, line, flags=re.IGNORECASE)

            elif (re.match("^" + bsdcvWordMeaning, line)):
                # Re-format bsdcvWordMeaning
                replacedStr = re.sub("^(" + bsdcvWordMeaning + ")",
                        identifierSubStr, line, flags=re.IGNORECASE)

            elif (re.match("^" + bsdcvWordExample, line)):
                # Re-format bsdcvWordExample
                replacedStr = re.sub("^(" + bsdcvWordExample + ")",
